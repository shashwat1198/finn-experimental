...
dql_add = make_node(
    "DequantizeLinear", inputs=["clp_s_t_add","scale_all","zero_point_all"], outputs=["dql_s_t_add"], name="dql_add"
    )
id_node_1 = make_node(
    "Identity", inputs=["dql_s_t_add"], outputs=["dql_s_t_add_out"]
)
tanh_s = make_node(
    "Tanh", inputs=["dql_s_t_add"], outputs=["s_t"],name="tanh_s"
)
ql_act = make_node(
    "QuantizeLinear", inputs=["s_t","scale_sigmoid","zero_point_sigmoid"], outputs=["ql_s_t"], name="ql_act"
    )
# id_node_1 = make_node(
#     "Identity", inputs=["ql_s_t"], outputs=["ql_s_t_out"]
# )
clp_act = make_node(
    "Clip", inputs=["ql_s_t","min","max"], outputs=["clp_s_t"], name="clp_act"
    )
dql_act = make_node(
    "DequantizeLinear", inputs=["clp_s_t","scale_all","zero_point_all"], outputs=["dql_s_t"], name="dql_act"
    )
id_node_2 = make_node(
...
onnx.save(onnx_model_14,'./lstm-gate-tanh.onnx')

...
#                                     dtype=np.float32)
int_input_range = np.arange(start= - 1,stop= 1, step = 254, dtype=np.float32)
...
print('Input thresholds : Unique inputs for which there is a change in the output')
print(thresholds)
...
while modified_index < 255:
    if threshold_index + 1 == len(thresholds):
        modified_thresholds[modified_index] = int_input_range[unique_input_indices[index_pos+1]]
        modified_index += 1
    elif thresholds[threshold_index] == thresholds[threshold_index+1]: 
        threshold_index += 1
    elif thresholds[threshold_index] != thresholds[threshold_index+1]:
        diff = thresholds[threshold_index+1] - thresholds[threshold_index] # Calculating number of required repeats

#In this file we convert the 'Tanh + QuantLinear' operations to 'MultiThresholding op'
...
#Final clipping values
clip_min = -128
clip_max = 127

#Quantization operation parameters
#scale = 0.0078125 #128
scale = 0.003921568859368563 # 256
#zero_point = -128 #The zero_point for tanh by default is set to -128 as it's range is [-1,1] and thresholding cannot give negative values as outputs.
zero_point = 0

if activation == 'Tanh':
    activation_out = nn.Tanh()
elif activation == 'Sigmoid':
    activation_out = nn.Sigmoid()
else:
    activation_out = nn.Linear()

int_input_range = np.arange(start= - 2 ** (acivation_bit_width - 1), stop= 2 ** (acivation_bit_width - 1), dtype=np.float32)
...
output = activation_out(int_input_range)   #output of the activation function
print(activation+'_out')
print(output) 
...
print('-------- These are the thersholds -------------------------')
...
#MT comparison operation : Can do this with more efficient techniques like Binarysearch
print('MT output')
for i in range(num_inputs):
    ...
    while modified_thresholds[start_index] <= int_input_range[i]: #and start_index < 254
        ...
        if start_index == 255:
            break
    out_array[i] =  np.minimum(start_index,clip_max) #For INT8 QuantLinear; clipping the output at 127

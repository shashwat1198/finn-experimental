...
class _QuantLSTMCell(nn.Module):
    ...
    def forward_iter(
        ...
        np.save('./quant_ii_gate.npy',quant_ii_gate.detach().numpy())
        print('quant_input_supplied to brevitas = ',quant_input)
        quant_hi_gate = F.linear(quant_hidden_state, quant_weight_hi)
        np.save('./quant_hi_gate.npy',quant_hi_gate.detach().numpy())
        quant_input_gate = self.input_acc_quant(quant_ii_gate + quant_hi_gate + quant_bias_input)[0]
        np.save('./quant_input_acc_gate.npy',quant_input_gate.detach().numpy())
        quant_input_gate = self.input_sigmoid_quant(quant_input_gate)[0]
        np.save('./quant_input_gate.npy',quant_input_gate.detach().numpy())
        # Forget gate
        if self.cifg:
            ...
            # print('quant_if_gate = ', quant_if_gate)
            np.save('./quant_if_gate.npy',quant_if_gate.detach().numpy())
            quant_hf_gate = F.linear(quant_hidden_state, quant_weight_hf)
            # print('quant_hf_gate = ', quant_hf_gate)
            np.save('./quant_hf_gate.npy',quant_hf_gate.detach().numpy())
            quant_forget_gate = self.forget_acc_quant(quant_if_gate + quant_hf_gate + quant_bias_forget)[0]
            np.save('./quant_forget_acc_gate.npy',quant_forget_gate.detach().numpy())
            quant_forget_gate = self.forget_sigmoid_quant(quant_forget_gate)[0]
            np.save('./quant_forget_gate.npy',quant_forget_gate.detach().numpy())
        # Cell gate
        quant_ic_gate = F.linear(quant_input, quant_weight_ic)
        np.save('./quant_ic_gate.npy',quant_ic_gate.detach().numpy())
        quant_hc_gate = F.linear(quant_hidden_state, quant_weight_hc)
        np.save('./quant_hc_gate.npy',quant_hc_gate.detach().numpy())
        quant_cell_gate = self.cell_acc_quant(quant_ic_gate + quant_hc_gate + quant_bias_cell)[0]
        np.save('./quant_cell_acc_gate.npy',quant_cell_gate.detach().numpy())
        quant_cell_gate = self.cell_tanh_quant(quant_cell_gate)[0]
        # print('quant_cell_gate = ', quant_cell_gate)
        np.save('./quant_cell_gate.npy',quant_cell_gate.detach().numpy())
        print('----------------------------')
        # Output gate
        quant_io_gate = F.linear(quant_input, quant_weight_io)
        np.save('./quant_io_gate.npy',quant_io_gate.detach().numpy())
        quant_ho_gate = F.linear(quant_hidden_state, quant_weight_ho)
        np.save('./quant_ho_gate.npy',quant_ho_gate.detach().numpy())
        quant_out_gate = self.output_acc_quant(quant_io_gate + quant_ho_gate + quant_bias_output)[0]
        np.save('./quant_out_acc_gate.npy',quant_out_gate.detach().numpy())
        quant_out_gate = self.output_sigmoid_quant(quant_out_gate)[0]
        np.save('./quant_out_gate.npy',quant_out_gate.detach().numpy())
        ...
        # return quant_hc_gate, quant_out_gate
    ...
    def forward(
        ...
        for _ in range(end):
            ...
            # quant_input_state = quant_input_states[0]

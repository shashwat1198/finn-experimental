...
w1_out = make_tensor_value_info("w1_out", onnx.TensorProto.FLOAT, [20,10])
...
# id_node_1_w = make_node("Identity", inputs=["W_f"], outputs=["w1_out"], name="id_node_1_w")
dql_w1 = make_node("DequantizeLinear", inputs=["ql_wf_out","scale_f","zero_point_all"], outputs=["dql_wf_out"], name="dql_w1")
...
dql_w2 = make_node("DequantizeLinear", inputs=["ql_wi_out","scale_i","zero_point_all"], outputs=["dql_wi_out"], name="dql_w2")
...
dql_w3 = make_node("DequantizeLinear", inputs=["ql_wc_out","scale_c","zero_point_all"], outputs=["dql_wc_out"], name="dql_w3")
...
dql_w4 = make_node("DequantizeLinear", inputs=["ql_wo_out","scale_o","zero_point_all"], outputs=["dql_wo_out"], name="dql_w4")
...
dql_u1 = make_node("DequantizeLinear", inputs=["ql_uf_out","scale_f","zero_point_all"], outputs=["dql_uf_out"], name="dql_u1")
...
dql_u2 = make_node("DequantizeLinear", inputs=["ql_ui_out","scale_i","zero_point_all"], outputs=["dql_ui_out"], name="dql_u2")
...
dql_u3 = make_node("DequantizeLinear", inputs=["ql_uc_out","scale_c","zero_point_all"], outputs=["dql_uc_out"], name="dql_u3")
...
dql_u4 = make_node("DequantizeLinear", inputs=["ql_uo_out","scale_o","zero_point_all"], outputs=["dql_uo_out"], name="dql_u4")
...
lstm_scan = make_graph(
    ...
    outputs = [out_input_gate,out_forget_gate,out_cell_gate,out_out_gate,out_hidden_state, out_cell_state, out_hidden_state_concat],
    value_info=[
            make_tensor_value_info("w1_out",onnx.TensorProto.FLOAT, [20,10]),
    ...
    initializer=[make_tensor('W_f',onnx.TensorProto.FLOAT, [20,10], (Wf_val)),
               ...
               #   make_tensor('second_add',onnx.TensorProto.FLOAT, [], [0]),
...
onnx.save(onnx_model, './quantize-lstm-full-graph-test-without-clip.onnx')
...
print(output[0])
print('------------------------------')
out_input_gate = output[0].reshape([1,20])
out_forget_gate = output[1].reshape([1,20])
out_cell_gate = output[2].reshape([1,20])
out_out_gate = output[3].reshape([1,20])
# print('Forget gate = ',output[0].reshape([1,20]))
# print('Input gate = ',output[1].reshape([1,20]))
# print('Cell gate = ',output[2].reshape([1,20]))
# print('Output gate = ',output[3].reshape([1,20]))
...
my_hidden_out = output[4].reshape([1,20])
...
# print('Brevitas_Input = ',input_out[0])
# print('Input = ',out_input_gate[0])
# print('------------------------------')

# print('Brevitas_Output = ',forget_out[0])
# print('Output = ',out_out_gate[0])
# print('------------------------------')

# print('Brevitas_Forget = ',cell_out[0])
# print('Forget = ',out_forget_gate[0])
# print('------------------------------')

# print('Brevitas_Cell = ',out_out[0])
# print('Cell = ',out_cell_gate[0])
# print('------------------------------')
...
scale_sigmoid = 0.0078125
scale_tanh  = 0.003921569
...
# print(out_input_gate.shape)
diff_hidden = np.round((my_hidden_out - hidden_out[0])/scale_hidden)
diff_input = np.round((out_input_gate - input_out)/scale_sigmoid)
diff_forget = np.round((out_forget_gate - forget_out)/scale_sigmoid)
diff_cell = np.round((out_cell_gate - cell_out)/scale_sigmoid)
diff_out = np.round((out_out_gate - out_out)/scale_tanh)
...
print('Diff i = ', diff_input)
print('Diff f = ', diff_forget)
print('Diff c = ', diff_cell)
print('Diff o = ', diff_out)
...
# out_d = make_tensor_value_info("out_d", onnx.TensorProto.FLOAT, [20,10])#c_t
...
# optimized_lstm_scan_node_model = onnxoptimizer.optimize(lstm_scan_node_model, passes)

# onnx.save(optimized_lstm_scan_node_model, './lstm_scan_node_model_without_clip.onnx')
...
#Executing the onnx model here.
print("Are the initializers removed here?")
